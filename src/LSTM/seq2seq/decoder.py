...
class Decoder:
    def __init__(self, vocab_size: int, embedding_dim: int, hidden_dim: int, attention_dim: int):
        ...
        self.LSTM_cell = S2SCell(input_dims = embedding_dim + hidden_dim, hidden_dims = hidden_dim, output_dims = hidden_dim)
    ...
    def forward_step(self, token_id: int, cell_state: Tensor, 
        ...
        Xt, ft, it, ct, ot, tanh_cs, new_cell, new_hidden = self.LSTM_cell.forward(cell_state, hidden_state, cell_input)
        ...
        cache_entry = {
            ...
            "hidden_state": new_hidden, "context_vector": context, 
            "attn_alphas": alphas, "attn_activated": activated
        }
    ...
    def forward(self, encoder_total_hidden: Tensor, encoder_hidden: Tensor, 
        ...
        for t in range(T-1):
            ...
            logit, hidden_state, cell_state, cache_entry = self.forward_step(curr_token, cell_state, hidden_state, encoder_total_hidden)
            logits.append(logit)
    ...
    def backward(self, d_logits: Tensor, encoder_outputs: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        for t in range(T-1, -1, -1):
            ...
            prev_hidden = state_at_t['prev_hidden']
            token_id = state_at_t['token']
            alphas = state_at_t['attn_alphas']
            activated = state_at_t['attn_activated']
    ...
    def generate(self, encoder_hidden: Tensor, encoder_cell: Tensor,
        ...
        for _ in range(max_length):
            logits, hidden_state, cell_state, _ = self.forward_step(curr_token, cell_state, hidden_state, encoder_output)
    ...
    def sgd_step(self, learning_rate: float) -> None:
        ...
        self.attention.sgd_step(learning_rate)
    ...
    def save_weights(self, path: str) -> None:
        torch.save({
            ...
            'W_out': self.W_out,
            'b_out': self.b_out,
            'attention_V': self.attention.V, 
            'attention_W': self.attention.W, 
            'attention_U': self.attention.U, 
        }, path)
    ...
    def load_weights(self, path: str) -> None:
        ...
        self.attention.V = state['attention_V']  
        self.attention.W = state['attention_W']
        self.attention.U = state['attention_U']  